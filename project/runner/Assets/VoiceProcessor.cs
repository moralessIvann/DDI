...
//nk9qq43S9BbOOgL1uXxPWt5UVJlJa6T-hGkzYm6FfSW7
    ...
    public class VoiceProcessor : MonoBehaviour
    {
        ...
        public bool Active
        {
            ...
            set
            {
                ...
                {
                    _service.RecognizeModel = (string.IsNullOrEmpty(_recognizeModel) ? "en-ES_BroadbandModel" : _recognizeModel);//es-ES_BroadbandModel
                ...
                foreach (var res in result.results)
                {
                    foreach (var alt in res.alternatives)
                    {
                        ...
                        foreach(var word in alt.transcript.Split(' '))
                        {
                            ...
                            {
                                Debug.Log("comando detectado: " + word);
                                onVoiceCommandRecognized?.Invoke(word);
                                break;
                            }
                        }
                        
  /*
                        if(alt.transcript.Contains("salta"))
                        {
                            Debug.Log("comando detectado: salta");
                            FindObjectOfType<PlayerController>().Jump();
                            break;
                        }
                    
                        if(alt.transcript.Contains("derecha"))
                        {
                            Debug.Log("comando detectado: derecha");
                            FindObjectOfType<PlayerController>().TurnRightVoiceCommand();
                            break;
                        }
                        if(alt.transcript.Contains("izquierda"))
                        {
                            Debug.Log("comando detectado: salta");
                            FindObjectOfType<PlayerController>().TurnLeftVoiceCommand();
                            break;
                        }
*/
                        //*************************
